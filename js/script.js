// Primeiramente, começo defininfo as variáveis mais imporatntes: exibição e botão
// depois disso, faço a checagem de erro; vejo se está vazio
// depois crio a função assíncrona para começar o consumo da API
// Depois crio a função de exibição, mapeando os tipos de pokemon
// Fim
...
function UpperCaseforFirst(palavra){
    ...
    const PrimeiraLetra = palavra.substring(0,1) // Guardei a primeira letra
    const RestoPalavra = palavra.substring(1, TamanhoPalavra); // peguei o resto da palavra

    const LetraUpper = PrimeiraLetra.toUpperCase()

    const palavraFormatada = LetraUpper + RestoPalavra; // concatenei 
